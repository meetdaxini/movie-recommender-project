...
def get_trending_from_tmdb():
    ...
    for result in trend_movies_dicts['results']:
        trend_movie = [{'Title' : result['title'], 'Poster' :  'http://image.tmdb.org/t/p/w300' + result['poster_path'], 'Year' : result['release_date'], 'imdbRating' : 0, 'rotten_tomatoes' : 0}]
...
def recommender(request):
    if 'movie' in request.GET:  
        ...
        #from api
            ...
            if not added_movie:
                params = {'movie_name' : movie, 'no_movie' : "No Movie found for you query '" + movie + "'"} 
                return render(request, 'recommender/recommender.html', params)
            search_movie = Movie(imdbID=added_movie[0], Title=added_movie[1], Poster=added_movie[2], Year=added_movie[3][:4], imdbRating=added_movie[4], rotten_tomatoes=added_movie[5])
            search_movie.save()    
        ...
        for title in get_movie_recommendations:
            #from db
            try:
                db_related_mv = Movie.objects.get(Title=title)
                # print(db_related_mv.Title, 'found related in  db')
                related_movies += [db_related_mv]        

            except MultipleObjectsReturned:
                db_related_mv = Movie.objects.filter(Title__iexact=movie).order_by('-imdbRating').first()
                related_movies += [db_related_mv]        
            ...
            #from api
                ...
                if related_movie_data:
                    new_related_mv = Movie(imdbID=related_movie_data[0], Title=related_movie_data[1], Poster=related_movie_data[2], Year=related_movie_data[3][:4], imdbRating=related_movie_data[4], rotten_tomatoes=related_movie_data[5])
                    new_related_mv.save()
        ...
        params = {'movie_name' : movie, 'search_movie' : search_movie, 'many_movies' : related_movies} 
        return render(request, 'recommender/recommender.html', params)
    else:
